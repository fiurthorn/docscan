...
class StateParameter with _$StateParameter {
  const factory StateParameter({
    ...
    @Default(false) bool showPdfSorter,
...
class ScannerBloc extends ReactiveBloc<StateParameter> implements GoRouteAware {
  ...
  Future<void> loading() async {
    ...
    converter.updateValue(converterItems().firstWhere((item) => item.technical == original));
  }
  ...
  void storeScanResult(List<String> value, {bool add = false}) {
    if (value.isNotEmpty) {
      converter.updateValue(converterItems().firstWhere((item) => item.technical == original));
      usecase<ReadFilesResult, ReadFilesParam>(ReadFilesParam(value)).then((value) {
        List<FileAttachment> scannedImages = add ? List.of(state.parameter.scannedImages) : [];
        scannedImages.addAll(value.map((e) => FileAttachment(e.name, e.data, e.data.length)));

        List<Uint8List?> cachedImages = add ? List.of(state.parameter.cachedImages) : [];
        cachedImages.addAll(value.map((e) => e.data));
  ...
  void down() => _swap(state.parameter.currentScannedImage, 1);
  void up() => _swap(state.parameter.currentScannedImage, -1);

  void _swap(int index, int incr) {
    final scannedImages = List.of(state.parameter.scannedImages);
    final cachedImages = List.of(state.parameter.cachedImages);

    scannedImages.insert(index + incr, scannedImages.removeAt(index));
    cachedImages.insert(index + incr, cachedImages.removeAt(index));
  ...
  Future<({Uint8List data, int index})> convertedImage(int index) {
  ...
  void updateConvertedImage() async {
    progress.updateValue(0);
    emitProgress(progress: progress, max: state.parameter.scannedImages.length);
    ...
    await Future.forEach(
      ...
      (index) => convertedImage(index).then(
        (value) {
          cachedImages[value.index] = value.data;
          progress.updateValue(value.index);
          return Log.norm("converted #${value.index + 1}");
        },
      ),
    )
        .then((value) => emitProgressSuccess())
        .then((value) => emitUpdate(parameter: parameter.copyWith(cachedImages: cachedImages)));

    // final index = state.parameter.currentScannedImage;
    // currentScannedImageSink.add(null);

    // if (state.parameter.cachedImages[index] != null) {
    //   currentScannedImageSink.add(state.parameter.cachedImages[index]!);
    // }

    // final item = state.parameter.scannedImages[index];
    // usecase<ConvertImageResult, ConvertImageParam>(
    //   ConvertImageParam(
    //     converter: converter.value!.technical,
    //     itemName: item.name,
    //     itemData: item.data,
    ...
    //       ),
    //     )
    //     .then((_) => currentScannedImageSink.add(state.parameter.cachedImages[index]!));
  }
...
//   }
  ...
  void amountChanged() async {
    updateConvertedImage();
  }

  void thresholdChanged() async {
    updateConvertedImage();
  }

  updateConverter(FormControl<I18nLabel> value) async {
    updateConvertedImage();
  }
